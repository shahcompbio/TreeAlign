...
class CloneAlignTree(CloneAlign):
    ...
    # make TreeAlign tree output easier for downstream analysis
    def format_tree_output(self, clone_assign_tree):
        # get clean clone assign tree results
        clone_assign_tree = clone_assign_tree.rename(columns={'clone_id': 'clonealign_tree_id'})        
        clone_assign_tree, pie_chart = TreeFormatter.clean_tree_based_clonealign_output(self.tree, clone_assign_tree)

        cnv_cells = pd.DataFrame({'cell_id': [terminal.name for terminal in self.tree.get_terminals() if terminal.name in self.cnv_df.columns]})        
        # get terminal nodes
        terminal_nodes = []
        for entry in pie_chart:
            if len(entry["value"]) == 1:
                terminal_nodes.append(entry["name"])

        # get cnv cell assignments
        cnv_clone_assign = TreeFormatter.get_cnv_cell_assignments(clone_assign_tree, self.tree, cnv_cells)     

        # filter cnv
        clones = set(cnv_clone_assign['clonealign_tree_id'].unique().tolist())
        clone_assign_tree = clone_assign_tree[clone_assign_tree['clonealign_tree_id'].isin(clones)]          
        return cnv_clone_assign, clone_assign_tree
    ...
    def assign_cells_to_clade(self, current_clade, expr_cells, level):
        ...
        if self.record_input_output:
            ...
            self.params_dict[current_clade.name]['output']['losses'] = losses
            self.params_dict[current_clade.name]['output']['times'] = times
